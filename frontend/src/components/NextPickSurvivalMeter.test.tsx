...
describe('survivalBand', () => {
  it('uses half-open 20% buckets, with 1.0 in fs', () => {
    expect(survivalBand(0).id).toBe('hell-nah');
    expect(survivalBand(0.199).id).toBe('hell-nah');
    expect(survivalBand(0.2).id).toBe('nah');
    expect(survivalBand(0.399).id).toBe('nah');
    expect(survivalBand(0.4).id).toBe('shiii-mayb');
...
describe('NextPickSurvivalMeter', () => {
  it('renders the percent, meme labels, and meter semantics', () => {
    ...
    expect(screen.getByText('hell nah')).toBeInTheDocument();
    expect(screen.getByText('nah')).toBeInTheDocument();
    expect(screen.getByText('shiii mayb')).toBeInTheDocument();
    expect(screen.getByText('yee')).toBeInTheDocument();
    expect(screen.getByText('fs')).toBeInTheDocument();
    const meter = screen.getByRole('meter');
    expect(meter).toHaveAttribute('aria-valuenow', '42');
    expect(meter).toHaveAttribute('aria-valuetext', '42 percent, shiii mayb');
    expect(meter).toHaveAttribute('data-band', 'shiii-mayb');
  });
